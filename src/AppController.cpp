...
void AppController::acceptCrossing(size_t idx) {
    ...
    _lastCrossHh = _prevHh; _lastCrossMm = _prevMm;
    _lastCrossSs = _prevSs; _lastCrossCs = _prevCs;
...
void AppController::updateRaceStage() {
    ...
    switch (_stage) {
        ...
        case RaceStage::STOPPED:
            if (_gps->hasFix() && _gps->speedKmh() > AppConst::LOG_MOVING_MIN_KMH) {
                _log->startNewLog(_gps->timeValid(), _gps->year(), _gps->month(), _gps->day(),
                                   _gps->hour(), _gps->minute(), _gps->second());
                _stage = RaceStage::ACTIVE;
...
void AppController::updateDisplayModel() {
    ...
    model.loggingActive = _log->isLogging();
...
void AppController::loop() {
    ...
    _log->updateSpeed(_gps->hasFix() ? _gps->speedKmh() : 0.0f);
    _log->loop();
